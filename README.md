...
本项目主要采用 **手工功能测试 + Postman 接口测试 + SQL 数据验证 + JMeter 基础性能测试**，未使用 UI 自动化测试。
...
> 本项目没有真实开发修复版本，因此没有为了形式单独执行“回归测试”。若后续存在缺陷修复或版本变更，应先进行缺陷验证，再根据影响范围选择相关核心用例进行回归。
...
> API Token、数据库密码等敏感信息仅保存在本地环境中，不提交至 GitHub。
...
> 本项目为本地单机环境下的基础性能测试实践，不将测试结果作为生产环境容量或 SLA 结论。
